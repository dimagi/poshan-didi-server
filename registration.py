...
def start(update, context):
    ...
    send_text_reply(
        'Hello and welcome. My name is Poshan Didi and I am currently under development.', update)

    send_text_reply(
        f'Can I call you {update.message.from_user.first_name}?', update)
    return CONFIRM_NAME
...
def confirm_name(update, context):
    ...
    if intent == Intent.YES:
        ...
        send_text_reply(
            'Ok, no problem. What would you like me to call you (just a first name is fine)?', update)
        return ASK_NAME

    send_text_reply(
        f"I didn't quite get that. Please respond with 'yes' or 'no'. Can I call you {update.message.from_user.first_name}?", update)
    return CONFIRM_NAME
...
def ask_child_name(update, context):
    ...
    send_text_reply(
        ...
        f'What is the birthday for {update.message.text}? Please enter in this format: YYYY-MM-DD', update)
...
def thanks(update, context):
    ...
    send_text_reply(
        "Thank you! You're all registered. Just wait for your first message, which should come at some point in the next 7 days", update)
